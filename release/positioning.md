...
Control Safari from the keyboard with a private command palette, link hints, tab search, bookmark/history search, and site-aware shortcuts.
...
- Safari-focused command palette: one palette covers open tabs across windows, bookmarks with folder paths, recent history, local visits, extension commands, direct URLs, and web search.
...
- A command palette that replaces repeated tab, bookmark, history, and search workflows.
