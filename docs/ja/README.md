...
- `f` で、通常の `http` / `https` ページの現在の viewport に見えているリンク、安全な navigation menu trigger、境界付けられた media player control、password 以外の native form control、semantic custom control に、黄色背景・黒文字のコンパクトな Hint を表示します。
...
- `o` で browser navigation palette を開き、browser window をまたいで開いている tab、ローカルで観測した page、extension command、direct URL、設定した search engine での web search、Safari が runtime で API を公開している場合の bookmark / recent history を検索します。
- Default の `o` palette は、入力前でも open tab、ローカルで観測した destination、利用可能な場合は browser-provided bookmark / history suggestion を表示できます。
- `v` の history palette は Safari が history result を公開している場合に recent history を表示し、browser history が使えない、または空 query に結果を返さない場合はローカルで観測した page を fallback として表示します。
- History result は recency に加えて、上限付きの visit frequency boost でも順位付けされるため、よく開く destination に届きやすくなります。
- 空の `o` / `Shift+O` palette は recent palette query を表示でき、選ぶと palette を閉じずに入力欄へ戻して再検索できます。
- `ge` で command palette を開き、現在の page URL を `url: ...` として入力済みにして、開く前に編集できます。
- `gE` で command palette を開き、現在の page URL を `url: ...` として入力済みにし、activation の default を新しい前面 tab にできます。
...
- `Option+U` で、command palette を閉じずに選択中の destination の URL へ query を絞り込めます。
...
- `Ctrl+m` で `Enter` と同じように選択中の command palette result を実行できます。`Ctrl+[` / `Ctrl+g` で `Esc` と同じように palette を閉じられます。
- `Ctrl+i` で、`Tab` と同じように次の command palette result へ移動できます。
- `Ctrl+j` / `Ctrl+k` で command palette の selection を上下に移動できます。plain `j` / `k` は input への入力として扱われます。
- `Ctrl+f` / `Ctrl+v` と `Ctrl+b` で、input から focus を外さずに command palette の selection を 1 page 分上下へ移動できます。
- `Option+Up` / `Option+Down` で、最近の command palette query を呼び戻せます。
- `Ctrl+a` / `Ctrl+e` で、query を変えずに command palette input の caret を先頭または末尾へ移動できます。
- `Ctrl+l` で、palette を閉じずに command palette query 全体を選択できます。
- `Ctrl+u` で command palette を閉じずに現在の query を消去できます。
- `Ctrl+d` で command palette を閉じずに query の次の文字、または選択中の query text を削除できます。
- `Ctrl+h` で command palette を閉じずに query の直前の文字、または選択中の query text を削除できます。
- `Ctrl+w` で command palette を閉じずに query の直前の単語を削除できます。
- `Ctrl+r` / `Option+R` で command palette を閉じずに query を保ったまま result を再読み込みできます。
- `Option+A/T/B/H/V/S/M` で、入力中の query を保ったまま command palette source を all sources、tab、bookmark、history、local visit、search、command に切り替えられます。`Shift+Option+U` で direct URL generation に切り替えます。
- Safari が bookmark result を公開している場合は `b` で bookmark 専用 palette、`v` で local fallback destination 付きの recent history 専用 palette、`Shift+T` で open tab 専用 palette を開けます。`Shift+B` と `Shift+V` では、利用可能な bookmark / history result を新しい tab で開きます。
- Browser-provided bookmark result は folder path でも一致し、folder path を表示します。Safari が bookmark data を公開している場合、たとえば `work docs` で該当 folder 配下の bookmark を見つけられます。
...
- Safari が bookmark data を公開している場合、`b` / `Shift+B` で bookmark を現在 tab / 新しい前面 tab 向けに検索します。
- `Shift+T` で browser window をまたいで開いている tab を検索します。
- Open-tab palette result はブラウザが last-accessed time を返す場合にそれを使うため、最近使った tab に戻りやすくなります。
...
- `docs/release/app-store-metadata.md`: App Store listing と review-note draft。
- `docs/release/positioning.md`: product positioning と differentiation guidance。
...
- public manifest が local `file:` page access を要求していないことを確認する。将来 development-only manifest で local file support を戻す場合は、App Store build とは別にテストする。
...
- 通常のページフォーカスで `o` を押し、tab title、ローカルで観測した page、command name、URL、search term、または browser-provided bookmark/history result を入力して、`Enter` で選択結果が開くか実行されることを確認する。
- 日本語 IME が有効な状態で、IME composition が始まる `o` を押し、拡張機能が palette を開かず、IME 確定の `Enter` を消費しないことを確認する。
- 通常のページフォーカスで `o` を押し、Safari が bookmark data を公開している場合は、入力前の all-source palette に browser-provided recent bookmark suggestion が表示されうることを確認する。
- 通常のページフォーカスで `o` を押し、入力前の all-source palette に open tab が表示されうることを確認する。Safari が history data を公開している場合は recent history も表示されうることを確認する。
- palette 使用後に通常のページフォーカスで `o` を押し、入力前に recent query suggestion が表示されうること、選択すると palette が閉じずに入力欄へ入り result が更新されることを確認する。
- 通常のページフォーカスで `ge` を押し、palette が開いて入力欄に `url: <current page URL>` が入っていることを確認する。
- 通常のページフォーカスで `gE` を押し、palette が開いて入力欄に `url: <current page URL>` が入り、`Enter` で編集後の URL が新しい前面 tab に開くことを確認する。
...
- palette 内で `ArrowDown`、`ArrowUp`、`Ctrl+n`、`Ctrl+p`、`Ctrl+i`、`Tab`、`Shift+Tab` を押し、palette から focus が外れずに選択 result が移動することを確認する。`PageDown` / `PageUp` では大きめに移動し、`Home` / `End` では最初と最後の result へ移動し、長い result list でも選択行が見える位置に保たれることを確認する。
...
- palette 内で `Alt+U` / `Option+U` を押し、選択中の browser destination に対して palette を閉じずに入力が `url:<selected URL>` へ変わることを確認する。
...
- palette 内で `Ctrl+m` を押し、`Enter` と同じように選択中の result が実行されること、`Ctrl+[` / `Ctrl+g` で `Esc` と同じように palette が閉じることを確認する。
- palette 内で `Ctrl+i` が `Tab` と同じように次の result へ移動すること、`Ctrl+j` / `Ctrl+k` で選択中の result が上下に移動すること、plain `j` / `k` は input に入力されることを確認する。
- palette 内で `Ctrl+f` / `Ctrl+v` と `Ctrl+b` を押し、input に focus が残ったまま選択中の result が 1 page 分上下に移動することを確認する。
- palette 内で `Alt+Up` / `Option+Up` と `Alt+Down` / `Option+Down` を押し、最近の palette query を前後に移動できることを確認する。
- palette 内で `book: project docs` と入力し、`Ctrl+a` を押して query を変えずに caret が先頭へ移動することを確認する。続けて `Ctrl+e` を押し、caret が末尾へ移動することを確認する。
- palette 内で `history: project docs` と入力し、`Ctrl+l` を押して palette が開いたまま query 全体が選択されることを確認する。
- palette 内で query を入力し、`Ctrl+u` を押して、palette が開いたまま入力が消えることを確認する。
- palette 内で `book: project docs` と入力し、`Ctrl+a` に続けて `Ctrl+d` を押して、palette が開いたまま最初の文字が削除され result が更新されることを確認する。`project` を選択して `Ctrl+d` を押し、選択中の text が削除されることを確認する。
- palette 内で `book: project docs` と入力し、`Ctrl+h` を押して、palette が開いたまま直前の文字が削除され result が更新されることを確認する。`project` を選択して `Ctrl+h` を押し、選択中の text が削除されることを確認する。
- palette 内で `book: project docs` と入力し、`Ctrl+w` を押して、palette が開いたまま直前の単語が削除され result が更新されることを確認する。
- palette 内で `Ctrl+r` または `Alt+R` / `Option+R` を押し、現在の query が保たれたまま result list が再読み込みされることを確認する。
...
- 入力前に `v` を押し、Safari が history data を公開している場合は recent history candidate が表示されることを確認する。そうでない場合は、拡張機能で page を閲覧したあとローカルで観測した page が fallback candidate として表示されることを確認する。
- `Shift+O` を押し、URL または destination を入力して、`Enter` で新しい前面 tab に開くことを確認する。
- `b` / `Shift+B` を押し、Safari が bookmark data を公開している場合は、入力前から bookmark 候補が表示され、bookmark だけが検索され、`Shift+B` では選択した bookmark が新しい前面 tab に開くことを確認する。
