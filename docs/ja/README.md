...
- `Ctrl+m` で `Enter` と同じように選択中の command palette result を実行できます。`Ctrl+[` で `Esc` と同じように palette を閉じられます。
...
- palette 内で `Ctrl+m` を押し、`Enter` と同じように選択中の result が実行されること、`Ctrl+[` で `Esc` と同じように palette が閉じることを確認する。
