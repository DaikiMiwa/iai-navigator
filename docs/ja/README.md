...
- Palette の destination query では、`title:<term>` と `url:<term>` で対象 field を絞れます。`title:"project docs" url:github.com -url:archive` のように phrase や除外 filter と組み合わせられます。
...
- palette 内で `title:"project docs" url:github.com -url:archive` と入力し、title と URL field で destination result を絞れることを確認する。
