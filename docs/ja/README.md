...
- `o` で browser navigation palette を開き、開いている tab、bookmark、recent history、extension command、direct URL、web search を検索します。
- `Shift+O` で palette の結果を新しい前面 tab で開きます。
- `b` / `Shift+B` で bookmark を現在 tab / 新しい前面 tab 向けに検索します。
- `Shift+T` で開いている tab を検索します。
...
- 通常のページフォーカスで `o` を押し、tab title、bookmark title、history URL、command name、URL、search term を入力して、`Enter` で選択結果が開くか実行されることを確認する。
