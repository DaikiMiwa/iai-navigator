...
- Safari Start Page や browser chrome focus など、page-level shortcut が動かない場所では、browser-level fallback shortcut の `Option+Shift+J` と `Option+Shift+K` でタブを左右に移動します。
...
- Safari タブを 3 つ以上開いた状態で Safari Start Page または address bar に focus し、`Option+Shift+J` で左のタブへ、`Option+Shift+K` で右のタブへ移動できることを確認する。Safari が shortcut conflict を報告する場合は、Safari の extension shortcut settings で同じ command を割り当ててから再確認する。
