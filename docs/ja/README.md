...
- `f` で、通常の `http` / `https` ページの現在の viewport に見えているリンク、安全な navigation menu trigger、境界付けられた media player control、native form control、semantic custom control に、黄色背景・黒文字のコンパクトな Hint を表示します。
