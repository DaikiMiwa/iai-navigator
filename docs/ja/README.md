...
- `Option+1` から `Option+9` で、対応する表示中の command palette result を開けます。`Shift` を加えると numbered result を新しい前面 tab で開きます。
...
- palette 内で `Alt+1` / `Option+1` から `Alt+9` / `Option+9` を押し、対応する表示中の numbered result が開くことを確認する。`Shift+Alt+1` / `Shift+Option+1` で最初の result が新しい前面 tab で開くことを確認する。
