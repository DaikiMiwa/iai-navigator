...
- `f` で、通常の `http` / `https` ページの現在の viewport に見えているリンクと native form control に、黄色背景・黒文字のコンパクトな Hint を表示します。
- リンクの Hint を最後まで入力すると、そのリンクの通常のクリック動作を現在タブで実行します。
- form control の Hint を最後まで入力すると、テキスト入力系は focus し、button、checkbox、radio、select などは通常の click/focus 挙動を実行します。
...
- `f` で見えているリンクと native form control に Hint が表示される。
- text input と textarea の Hint を最後まで入力すると focus される。
