...
拡張機能は、有効な状態で観測した page の bounded な local list も保存します。各 entry に含まれるのは page URL、page title、最後に観測した時刻、visit count だけです。この local index により、Safari が runtime で bookmark や history API を公開しない場合でも、command palette から destination を探しやすくします。
