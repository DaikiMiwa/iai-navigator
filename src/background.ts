((
  ...
  async function searchPalette(
    ...
    const historyResults =
      sources.has("history") && !trimmedQuery && history.length === 0
        ? localVisitsAsHistoryItems(
            visits.length > 0
              ? visits
              : api.storage?.local
    ...
    return {
      results: searchPaletteResults(
        { bookmarks, history: historyResults, tabs, visits },
        message.query,
        {
  ...
  async function activeCurrentWindowTab(
    api: WebExtensionApi,
  ): Promise<WebExtensionTab | null> {
    if (!api.tabs) {
      return null;
    }
  ...
  function isTabCommandMessage(message: unknown): message is TabCommandMessage {
    if (!message || typeof message !== "object") {
      return false;
    }

    const candidate = message as Partial<TabCommandMessage>;
    return (
      candidate.type === "tab-command" &&
  ...
  function historyPaletteResult(
    ...
    const frequencyBoost =
      typeof historyItem.visitCount === "number"
  ...
  function localVisitsAsHistoryItems(
    visits: LocalVisitItem[],
  ...
  function paletteQueryTerms(query: string): PaletteQueryTerm[] {
    const terms: PaletteQueryTerm[] = [];
    let index = 0;
