...
((global) => {
    ...
    function searchPaletteResults(sources, query, options = {}) {
        ...
        const bookmarkResults = sourceFilter.has("bookmarks")
            ? bookmarkPaletteResultsForQuery(sources.bookmarks, normalizedQuery, sourceFilter.size === 1
                ? { emptyQueryMode: "all" }
                : {
        ...
        const results = [
            ...
            ...(options.includeGenerated
                ? generatedPaletteResults(normalizedQuery, options.searchEngine, options.generatedKinds, options.customSearchUrlTemplate)
                : []),
        ];
    ...
    async function searchPalette(api, message) {
        ...
        const [tabs, bookmarks, history, visits, searchSettings] = await Promise.all([
    ...
    function tabPaletteResult(tab, query) {
        ...
        const recencyBoost = typeof tab.lastAccessed === "number"
    ...
    function isAllSourcePalette(sources) {
        const sourceSet = new Set(sources);
        return (sourceSet.has("tabs") &&
            sourceSet.has("bookmarks") &&
            sourceSet.has("history") &&
            sourceSet.has("visits"));
    }
    function bookmarkPaletteResultsForQuery(bookmarks, query, options) {
        const entries = bookmarks.flatMap((bookmark) => bookmarkPaletteResultEntries(bookmark, query));
    ...
    function compareBookmarkPaletteResultEntriesByRecent(a, b) {
        if (a.dateAdded !== b.dateAdded) {
            return b.dateAdded - a.dateAdded;
        }
        return a.result.title.localeCompare(b.result.title);
    }
    function bookmarkPaletteResultEntries(bookmark, query, folders = []) {
        if (!bookmark.url) {
            const nextFolders = bookmark.title
                ? [...folders, bookmark.title]
                : folders;
            return (bookmark.children ?? []).flatMap((child) => bookmarkPaletteResultEntries(child, query, nextFolders));
        }
        ...
        return [
            {
                dateAdded: typeof bookmark.dateAdded === "number" &&
    ...
    function generatedPaletteResults(query, searchEngine = DEFAULT_SEARCH_ENGINE, generatedKinds = PALETTE_GENERATED_KINDS, customSearchUrlTemplate = "") {
        if (!query) {
            return [];
        }
        const results = [];
        ...
        if (kindFilter.has("search")) {
            const engine = searchEngineConfig(searchEngine, customSearchUrlTemplate, query);
            results.push({
                id: `search:${query}`,
                kind: "search",
    ...
    function searchEngineConfig(searchEngine, customSearchUrlTemplate, query) {
        if (searchEngine === "custom") {
            const customSearchUrl = customSearchUrlFromTemplate(customSearchUrlTemplate, encodeURIComponent(query));
            if (customSearchUrl) {
                return { label: "Custom Search", searchUrl: customSearchUrl };
            }
        }
        const engine = searchEngine === "custom"
    ...
    function paletteFieldTermScore(term, fieldValue, includeScore) {
    ...
    async function loadConfiguredSearchSettings(api) {
        ...
        return {
            customSearchUrlTemplate: typeof commandPalette?.customSearchUrlTemplate === "string"
