...
test("maps command palette navigation keys", () => {
  ...
  assert.equal(
    commandPaletteKeyAction(key({ ctrlKey: true, key: "v" })),
    "page-next",
  );
  assert.equal(
    commandPaletteKeyAction(key({ ctrlKey: true, key: "V" })),
  ...
  assert.equal(
    commandPaletteKeyAction(key({ ctrlKey: true, key: "a" })),
    "move-query-start",
  );
  assert.equal(
    commandPaletteKeyAction(key({ ctrlKey: true, key: "A" })),
...
test("maps command palette activation keys", () => {
  ...
  assert.equal(
    commandPaletteKeyAction(key({ ctrlKey: true, key: "r" })),
    "refresh-results",
  );
  assert.equal(
    commandPaletteKeyAction(key({ ctrlKey: true, key: "R" })),
...
test("describes command palette activation and source-prefix hints", () => {
  ...
  assert.match(hints, /Ctrl\+F\/V\/B/);
  assert.match(hints, /Ctrl\+A\/D\/E\/H\/L\/U\/W/);
  assert.match(hints, /Ctrl\+R\/Option\+R/);
  assert.match(hints, /Option\+↑\/↓/);
...
test("deletes the previous command palette character", () => {
  assert.deepEqual(
  ...
  assert.deepEqual(
    ...
    { selectionEnd: 0, selectionStart: 0, value: "docs" },
  );
  assert.deepEqual(
    commandPaletteDeletePreviousCharacterValue({
      selectionEnd: 999,
...
test("moves the command palette input caret without changing query text", () => {
  const query = "tab: project docs";

  assert.deepEqual(commandPaletteMoveCaretRange(query, 0), {
    start: 0,
    end: 0,
  });
  assert.deepEqual(commandPaletteMoveCaretRange(query, query.length), {
    start: query.length,
    end: query.length,
  });
  assert.deepEqual(commandPaletteMoveCaretRange(query, -10), {
    start: 0,
    end: 0,
  });
  assert.deepEqual(commandPaletteMoveCaretRange(query, query.length + 10), {
    start: query.length,
    end: query.length,
  });
