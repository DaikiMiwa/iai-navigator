...
Private command palette、link hints、tab search、local destination search、available browser data search、site-aware shortcuts で Safari を keyboard から操作する。
...
- Safari-focused command palette: open tabs across windows、local visits、extension commands、direct URLs、web search、Safari が runtime で公開している場合の bookmark/history data を1つの palette で扱う。
...
- Repeated tab、local destination、available browser data、search workflows を置き換える command palette。
