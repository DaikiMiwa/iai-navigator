...
- `o` で browser navigation palette を開き、browser window をまたいで開いている tab、ローカルで観測した page、extension command、direct URL、設定した search engine での web search を検索します（Safari から利用可能な場合は browser-provided bookmark と history も対象になります）。
- デフォルトの `o` palette は、入力前から open tab、ローカルで観測した destination、および Safari から提供される bookmark/history suggestion（これらが利用可能な場合）を表示できます。
- `v` history palette は Safari が history result を返す場合にそれらを表示し、取得できない場合や空クエリで取得できない場合の fallback としてローカルで観測した page を表示します。
...
- `b` で Safari が bookmark 結果を公開している場合に入力前から候補が出る bookmark 専用 palette、`v` でローカル fallback 先がある recent history 専用 palette、`Shift+T` で open tab 専用 palette を開けます。`Shift+B` と `Shift+V` では現在利用可能な場合に bookmark と history の result を新しい tab で開きます。
- Browser-provided bookmark result は folder path でも一致し、folder path を表示します。Safari が bookmark data を公开している場合、たとえば `work docs` で該当 folder 配下の bookmark を見つけられます。
...
- `manual-test/frame-host.html` を開き、frame 内をクリックしたあと、`Shift+J/K` でタブ移動でき、`f` で frame 内 of Hint が表示される。
