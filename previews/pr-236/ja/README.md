...
- `v` の history palette は入力前から recent history を表示し、browser history が空 query に結果を返さない場合はローカルで観測した page を fallback として表示します。
...
- 通常のページフォーカスで `o` を押し、入力前の all-source palette に open tab と recent history が表示されうることを確認する。
...
- 入力前に `v` を押して recent history candidate が表示されることを確認する。Safari が空 query の browser history を返さない場合は、拡張機能で page を閲覧したあとローカルで観測した page が fallback candidate として表示されることを確認する。
